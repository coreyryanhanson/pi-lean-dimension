 ...
 * of cross-process storage-state flow for the Python adapter / firefox-py bridge.
 *
 * Uses a live Firefox browser via the Firefox-Py bridge against a local
...
const TASK_ID = "firefox-py-persistence";

// ═════════════════════════════════════════════════════════════════
// Tests — only run if Firefox + Python prerequisites are met
// ═════════════════════════════════════════════════════════════════

describeIfAvailable(
	"Firefox-Py cookie persistence across navigations",
	() => {
		const TEST_PROFILE = `firefox-py-persist-${Date.now()}`;

		let plugin: PythonPluginAdapter;

		beforeAll(async () => {
			plugin = new PythonPluginAdapter("firefox-py-persist", {
		...
		it("creates session and sets persistState (simulating the router's setup)", async () => {
			sessionManager.createSession(TASK_ID, "firefox-py-persist");
