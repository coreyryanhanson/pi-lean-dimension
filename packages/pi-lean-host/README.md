...
> `runMiniwobTask` / `benchPlugin` / public API for third-party
> plugin benchmarking.
...
configured to use. It requires the `playwright` Python package (any
modern version). No browsergym or dedicated venv needed.
...
|---|---|---|
| `runMiniwobTask` | `adapter/miniwob-adapter.ts` | Drive one episode; accepts `"trivial"` solver |
...
| _(removed)_ | BridgeClient tests deleted with the BrowserGym bridge replacement. |
...
`pi-lean-host` passes that CDP endpoint to the MiniWoB Python driver,
which attaches via `connect_over_cdp`. This tests the plugin's real launch path.

Used by: `chromium` (Phase 1).
...
and passes the endpoint to both the MiniWoB driver (Python) and the user plugin.
...
export interface BrowserPlugin {
  ...
  /** CDP endpoint for external attach (Mode A). null if the
   *  plugin doesn't expose one. */
