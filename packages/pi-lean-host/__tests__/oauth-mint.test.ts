 ...
 *  - token-URL confirm is the FIRST prompt, before picker + paste, on BOTH
 *    grant arms (prompt-order guarantee);
 *  - confirm / picker / paste cancel each throw the two-call `init … --code`
...
function makeToolCtx(
	...
	const ctx = {
		...
		ui: {
			...
			input: vi.fn(async (_title: string) => {
				orders.push("input");
				// Default: simulate the user having authorized and pasting the
				// address-bar URL with the pending flow's state.
				const pending = readPendingFlow("mint.invalid");
...
describe("oauth-mint — headless + store precheck", () => {
	...
	it("normalizes the store domain against provisioned parents", async () => {
		...
		expect(readToken("parent.invalid")?.accessToken).toBe("NORM");
		expect(readToken("api.parent.invalid")).toBeNull();
	});
});
...
describe("oauth-mint — token-URL confirm", () => {
	it("client_credentials: confirm shows the FULL tokenUrl + client name, fires before the exchange", async () => {
		...
		expect(readToken("mint.invalid")?.accessToken).toBe("CC");
		expect(fetchMock).toHaveBeenCalledTimes(1);
	});

	it("authorization_code: confirm fires before the paste prompt", async () => {
		writeSecret("mint.invalid", "client_id", "MY_CLIENT");
		stubTokenEndpoint(() =>
		...
		expect(m.orders[0]).toBe("confirm");
		expect(m.orders[0]).toBe("confirm");
		expect(m.orders.indexOf("confirm")).toBeLessThan(m.orders.indexOf("input"));
		expect(readToken("mint.invalid")?.accessToken).toBe("AC");
	});
...
describe("oauth-mint — scopes picker", () => {
	...
	it("picker cancel (Esc → undefined) throws the init escape-hatch hint with real values", async () => {
		...
		expect(readToken("pickcancel.invalid")).toBeNull();
	});
...
describe("oauth-mint — authorization_code arm", () => {
	it("paste prompt completes the flow; redirect URL never enters the transcript", async () => {
		...
		expect(readToken("mint.invalid")?.accessToken).toBe("PASTED");
		expect(readPendingFlow("mint.invalid")).toBeNull();
	...
	it("paste cancel (after retries) throws the init escape-hatch hint; the pending flow survives", async () => {
		...
		const m = makeToolCtx({ mode: "rpc" });
		(m.ctx.ui.input as ReturnType<typeof vi.fn>).mockResolvedValue(undefined);
		(m.ctx.ui.input as ReturnType<typeof vi.fn>).mockResolvedValue(undefined);
		await expect(
			oauthMintTool.execute(
				"test",
		...
		expect(readPendingFlow("mint.invalid")).not.toBeNull();
		expect(fetchMock).not.toHaveBeenCalled();
	});
});
