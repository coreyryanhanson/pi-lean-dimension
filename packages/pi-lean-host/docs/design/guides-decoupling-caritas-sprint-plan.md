# Guide Content Decoupling (`caritas`) — Sprint Plan

> Drafted as the executable sprint breakdown for
> [`guides-decoupling-caritas.md`](./guides-decoupling-caritas.md) (the design
> doc — the decision record for splitting the recipe/guide content out of
> `pi-lean-host` into a separate `caritas` repo). This doc is the *how/when*,
...
>
> The design doc's [Rollout / next steps](./guides-decoupling-caritas.md#rollout--next-steps)
...
- **The `caritas` repo** holding every non-axis recipe + `_shared/`
  (`test-harness.ts`, `probe-op.ts`) + `WAF-NOTES.md` + `CONTRIBUTING.md` +
  a README owning the perpetual drift disclaimer.
- **An `exports` map on `pi-lean-host`** so caritas can import framework
...
- **caritas CI** — per-PR (parse-validity + mocked-transport
...
| 4 | Add `exports` map to `pi-lean-host`; move non-axis content to `caritas` repo; migrate import paths | 3 | no |
| 5 | Stand up caritas CI (per-PR parse + mocked-transport; nightly live) + README drift disclaimer | 4 | no (per-PR) / yes (nightly) |
...
is gated on the move. The lockstep gate is gated on Sprint 5 (caritas nightly
...
[audit procedure](./guides-decoupling-caritas.md#deferred-axis-set-membership-tbd)
...
1. Enumerate the guide-driven framework feature axes from the design doc's
   [Test axes](./guides-decoupling-caritas.md#test-axes-the-audits-dimension-list)
...
2. Write a co-located mocked-transport test per axis guide: stub the
   ...
   the live `endpoint-coverage.test.ts` files move to caritas with the real
   recipes in Sprint 4.
...
## Sprint 4 — Add `exports` map; move non-axis content to `caritas`; migrate import paths

**Goal:** the actual split. Add an `exports` map to `pi-lean-host` so caritas
can import framework helpers as package imports, create the `caritas` repo,
...
**Gated on:** Sprint 3 (caritas is a devDep-only source repo — see the
design doc's Decisions).

**Local install, not npm publish:** caritas consumes `pi-lean-host` via a
...
caritas's local devDep resolves against the on-disk source (the `files`
...
land before 4c (move + import migration) can be verified, because caritas's
...
1. Add an `exports` field to `package.json` exposing the framework entry
   points caritas needs. The audited full set (run the 4c audit command —
   ...
   **No-build shape:** the package has `noEmit: true` (no `.js` files exist),
   ...
   `types`/`default` conditions map) so `tsx` and type-checking in caritas
   ...
   **Main entry (future-publish guard):** also add a `"."` entry pointing
   ...
   while caritas consumes a local-path devDep and imports specific
...
3. **Do not publish.** The `exports` map is a `package.json` edit only;
   caritas resolves it via a local install in 4b. The actual npm version
...
### 4b — Create the `caritas` repo

1. Create the `caritas` repo (separate repo, same license). Add a
   ...
   — **not** an npm version pin. No published caritas package — it is a
...
1. Move, from `pi-lean-host/api-guides/` to `caritas/api-guides/`:
...
4. In caritas, run the moved tests against the locally-installed
   `pi-lean-host`:
   - `all-guides-parse` equivalent over caritas's full recipe set (parse,
     no network).
...
5. **Update `__tests__/parse-api-guide.test.ts`** — the "real boe.es guide
   ...
   The real boe.es moves to caritas; the synthetic slim boe.es has far fewer
...
- `pi-lean-host` `package.json` carries the `exports` map (no npm publish
  ...
  **and** by caritas's local devDep resolving the moved tests'
...
- `caritas` repo exists with every non-axis recipe + `_shared/` +
  `WAF-NOTES.md` + `CONTRIBUTING.md`; all moved tests' imports rewritten to
  `pi-lean-host/core/...` and resolving against the devDep.
- In caritas: parse-validity over the full recipe set green (no network);
  moved mocked-transport `transform.test.ts`/`helper.test.ts` green.
- No `../../core/` relative imports remain in caritas (grep-clean).

---

## Sprint 5 — Stand up caritas CI + README drift disclaimer

**Goal:** give caritas the two-tier CI the design doc specifies — per-PR
...
**Gated on:** Sprint 4 (caritas repo exists with moved content + rewritten
imports).

### Tasks

1. **caritas per-PR CI (fast, gated, no network):**
   - Parse-validity: run `parseApiGuide()` over every guide (the
     `all-guides-parse` equivalent, now in caritas).
   ...
   - Both gated on PR status; a recipe PR that breaks parsing or helper
     wiring fails fast.
2. **caritas nightly CI (slow, non-gating, live):**
...
3. **README drift disclaimer** in `caritas/README.md` — the design doc's
...
4. Keep the `pi-lean-host` devDep in caritas as the local-path install from
   Sprint 4 during development. Bumping it is a deliberate caritas PR (so a
   schema change in host is a visible, reviewed event in caritas, not a
...
- caritas per-PR CI runs parse-validity + mocked-transport tests green on a
  clean PR; a deliberately-broken guide fails it.
- caritas nightly CI runs the live `HOST_INTEGRATION=1` suite on schedule;
  failures are non-gating (drift signal only).
- `caritas/README.md` carries the drift disclaimer, scoped to per-recipe
  `verified`-date provenance — distinct from any framework unstable
  disclaimer.
- `pi-lean-host` devDep present in caritas (local-path install during dev;
...
**Gated on:** Sprint 5 (caritas nightly live tests running as the drift
...
the README unstable disclaimer and caritas runs with its own drift
...
3. Remove the README unstable disclaimer (the pre-lockstep "compatibility is
   not guaranteed" block). **Do not** remove caritas's drift disclaimer —
...
`HOST_INTEGRATION` env-gate instructions (live tests now live in caritas).
Point users to caritas for the comprehensive recipe library.
...
1. `packages/pi-lean-host/AGENTS.md`:
   ...
   - Remove references to `_shared/test-harness.ts`, `_shared/probe-op.ts`,
     ...
     caritas). Note that the inline worked example in `tools/api-learn.ts`
     stays in host (separate from the full reference guides in caritas).
   - Update the `HOST_INTEGRATION` instructions: live-endpoint tests are no
     longer co-located in host; they live in caritas. Host's
     `api-guides/<domain>/` tests are mocked-transport, always-on, no env
     var.
   - Add a pointer to the `caritas` repo for the comprehensive recipe
...
- Axis-set description accurate; caritas pointer present.
...
- **No endpoint-recipe duplication.** Host's axis guides are synthetic
  minimal variants against mocked transport; caritas owns the real recipes
...
- **`schemaVersion` is attribution, not enforcement.** It never gates,
  ...
  set) and caritas's parse-validity CI are the only real gates. No parser
...
- **A published `caritas` npm package** — explicitly rejected; caritas is a
