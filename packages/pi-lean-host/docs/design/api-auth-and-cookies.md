...
- **`auth.requires: string[]`**: declares the secret names the guide
  expects (e.g. `auth.requires: [apiKey]` for static-key). Lives under
  the `auth` block alongside `auth.secretRefs` / `auth.secretQueryRefs`
  (not a top-level guide field), because it names auth secrets. The footer
  reads this to know what to look for in the store. **No `scopes` field.**
- **Parser/validator** enforces: literal-vs-reference is a **schema
  ...
  - **`auth.requires` ↔ referenced names:** every secret name appearing
    in `secretRefs` or `secretQueryRefs` must be declared in
    `auth.requires` (catches a typo'd `apiKey`/`api_key` mismatch at parse
    time, not at fetch time). `auth.requires` names with no referrer are
...
| Auth needed + credential present | `auth: ok` |
...
- **Auth headers on redirects → stripped on cross-domain.** The forced
  ...
  cross-domain redirect. Update the
...
- **Parser/validator tests** for the new auth fields: `auth.secretRefs`,
  `auth.secretQueryRefs`, `auth.requires`. Literal-vs-reference is a schema
  ...
  at parse with the "not yet implemented" fix), `auth.requires` ↔
  referenced-name validation (a `secretRefs`/`secretQueryRefs` name not
  in `auth.requires` is a parse error), and the `secretQueryRefs` ↔
...
- **Footer tests** — the auth-status states (none / auth-ok /
