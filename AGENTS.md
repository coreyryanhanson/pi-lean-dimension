...
Co-activation with `web-search` is **search-owned**: `pi-lean-search` mirrors `pi-lean-dimension.web` `TOOLSET_EVENTS.changed` events and enables/disables its own `pi-lean-dimension.search` toolset when `/web on|off` fires — portal has no reference to `web-search`. The mirror listens on `changed` only (not `restored`), so a stored `toolset-state:pi-lean-dimension.search` value survives sessions, and it skips mirroring while allowlist focus mode holds the line.
