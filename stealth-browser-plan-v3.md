...
- **Camoufox backend — DONE as a user-installed template.**
  `docs/stealth-backends/camoufox-py/bridge.py` is a complete,
  working, copy-pasteable subclass (committed). It is **docs, not
  source** — users drop it into `~/.pi/agent/pi-lean-portal/user-backends/`
  or use it as a reference for implementing other stealth browsers with
  quirks.
- **invisible-playwright — confirmed working, but NOT shipped.** The
  bridge exists only as an untracked local file
  (`docs/stealth-backends/invisible-py/bridge.py`, **not in git**).
  It validated the lifecycle-ownership-override pattern and the
  quirks schema against a second stealth engine, but
  ...
  do not want to take. **All references to it must be invisible to the
...
- **Remaining gaps (the work this plan owns):**
  ...
  3. **`docs/stealth-backends/README.md`** — install steps, venv setup,
     binary fetch, `settings.json` shape, absolute-`pythonPath` rule.
     Only `bridge.py` templates exist today; no README.
...
| Camoufox `bridge.py` template (docs) | ✅ | `docs/stealth-backends/camoufox-py/bridge.py` (committed `81cecb5`) |
...
1. **No invisible-playwright code in git.** The local
   `docs/stealth-backends/invisible-py/bridge.py` stays **untracked**
   (current state: `git status` shows it as `??`). It is a development
   artifact, not a shipped template.
2. **No references to `invisible-playwright` / `invisible-py` in
   shipped files.** This includes `AGENTS.md`, `STEALTH.md`, the
   `docs/stealth-backends/README.md`, the camoufox template's
   docstrings, test files, and CI workflows. Grep the tree before
   cutting a release: `grep -ri "invisible" packages/` outside the
   untracked file must return nothing.
...
Export a small probe from `pi-lean-host/src/index.ts` (re-export from
`miniwob-suite-helper.ts`) that resolves a user-managed backend's
bridge + venv and reports availability, mirroring the existing
`probePythonBackend` but rooted at `USER_BACKENDS_DIR`:
...
#### 1b. New shipped template: `docs/stealth-backends/camoufox-py/miniwob-parity.test.ts`
...
`packages/pi-lean-host/suites/miniwob-user-backends.test.ts` that:
...
> **`PythonPluginAdapter` constructor signature check:** before
> writing 1b/1c, verify `PythonPluginAdapter`'s constructor accepts
> `bridgePath` + `pythonPath` directly (the shipped suite files
> already construct it this way — see
> `miniwob-chromium-py.test.ts`). If the constructor instead takes a
> `PluginConfig`-shaped object, match that shape. The template must
> compile against the shipped adapter API.
...
- `docs/stealth-backends/camoufox-py/miniwob-parity.test.ts` template
...
- `packages/pi-lean-host/suites/miniwob-user-backends.test.ts` shipped,
  auto-skips when `user-backends/` is empty, and passes
  `npm run test:ci` (structural) green.
...
- **`PYTHONPATH` injection** making `pi_browser_bridge` importable from
  any user venv.
- **Camoufox as the shipped example template** (docs-only), with a
  one-line pointer to `docs/stealth-backends/camoufox-py/bridge.py`.
...
## Gap 3 — `docs/stealth-backends/README.md`
...
2. **Copy the template** from this repo's
   `docs/stealth-backends/camoufox-py/bridge.py` (or write your own
   subclass using the quirks schema documented in `playwright_base.py`).
...
9. **Benchmarking (optional):** pointer to the
   `docs/stealth-backends/camoufox-py/miniwob-parity.test.ts` template
   and the `registerMiniwobSuite` public API for running the 130-task
   MiniWoB++ suite against the user's install.

No mention of invisible-playwright.

---

## Gap 4 — `STEALTH.md` (or a `docs/stealth-backends/CHOOSING.md`)
...
- **When to use Camoufox specifically:** the shipped, tested template.
  ...
  `docs/stealth-backends/camoufox-py/` template.
...
- `camoufox-py.test.ts` — `runContractTests("camoufox-py", factory,
  { realBrowser: true, engine: "firefox" })`. The factory constructs a
...
`docs/stealth-backends/**`) job to `.github/workflows/ci.yml` that:
...
3. **Gap 2 (AGENTS.md)** + **Gap 3 (README)** + **Gap 4 (STEALTH.md)** —
   ...
   shipped.
4. **Gap 6 (CI)** — last; wires up everything from 1 + 5.
...
|------|----------|------------|
| `PythonPluginAdapter` constructor shape drift breaks the parity-test template | **Medium** | Verify the constructor signature against `miniwob-chromium-py.test.ts` before writing the template; compile-check the template in CI (docs-only lint step). |
...
| `grep -ri "invisible" packages/` leaks a reference in shipped code/docs | **Medium** | Add a CI grep guard (a tiny test asserting zero shipped-file matches for `invisible`) so the policy is enforced, not just documented. |
...
**Bottom line:** no blockers. The infra work (Phase 0/0b) and the
Camoufox template are done and tested. The remaining work is
MiniWoB++ integration for user-managed backends, docs, contract
tests, and CI — plus enforcing the invisible-playwright policy with a
grep guard.
