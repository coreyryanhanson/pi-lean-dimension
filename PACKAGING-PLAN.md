...
> (`pi-lean-search`) register **no** `/web` command — they're leaves that
...
keep the exact-name approach (no regex) and add a sibling set when search lands.
...
| `pi-lean-portal` + `pi-lean-search` | ✅ (portal owns it, single) | browser + search tools | Unified toggle, no `/web:1`/`/web:2`. |
| `pi-lean-dimension` (umbrella) | ✅ (bundles both above) | browser + search tools | Full-suite power-user option — requires SearXNG server + URL config (§3 Mode B). |
| `pi-lean-search` only | ❌ no `/web` | (search tool always on) | Search-only user has nothing to toggle. Acceptable — see §1.2. |

### 1.2 The one tradeoff (and why it's fine)

A **search-only user** (search installed without portal) gets **no `/web`** and
...
alongside it add `pi-lean-search` (Mode B-search) and portal's `/web on/off`
...
loads first wins `/web` — if search loads before portal, **search owns `/web`**
...
└── packages/
    ...
    ├── pi-lean-search/                 ← leaf: SearXNG search tool (web-search), no /web
...
| `pi-lean-search` | extension | no | (whatever it needs) | SearXNG search tool (`web-search`). Name lives in portal's `web-` namespace so `/web on/off` picks it up with no special-casing. |
| `pi-lean-dimension` | extension (umbrella) | no (delegates to bundled portal) | none directly | `bundledDependencies` of portal + search; `pi.extensions` references each via `node_modules/...` |
...
| `/web status` | portal | Lists all web tools by name with on/off state (current behavior, generalized to include search tools). Grouping by source package deferred to v0.2 (see §5.11, decision 12). |
...
const SIBLING_TOOL_NAMES = new Set<string>([
  // "web-search",  // ← added at search integration (step 5, decision 10)
]);
...
search ever renames its tool, portal's tests fail (the simulated `web-search`
...
> `pi install` and you're browsing. Dimension bundles search, which requires a running
> SearXNG server + a configured API URL — real setup friction most users won't
> want up front. Dimension is reframed as the **full-suite power-user option** for
...
later want search, `pi install npm:pi-lean-search` adds it (§3 Mode B-search) and
...
`pi-lean-dimension`'s `package.json`:

```jsonc
{
  "name": "pi-lean-dimension",
  "version": "0.1.0",
  "pi": {
    "extensions": [
...
configure a SearXNG URL (decision 15 — search degrades gracefully and prints
...
### Mode B-search — "Install search only" (requires SearXNG)

```bash
pi install npm:pi-lean-search          # search only, no /web
```
...
Ship a `/web-setup` command (in portal or dimension) that mirrors rpiv's
`/rpiv-setup`:
...
search, or vice versa. **v0.1 ships Mode A + Mode B + Mode B-search only; Mode C
lands in v0.2 if support requests come in.**

> **`pi-lean-search` peer-dep on portal.** `pi-lean-search` declares
...
> — the search tool works standalone; it just doesn't get a `/web` toggle). Do
...
### `pi-lean-search` (future — you've coded it)
...
by exact name). If search later adds `web-search-images` or `web-search-news`,
portal adds those exact names to `SIBLING_TOOL_NAMES` too. Portal must not add
a `web-search` tool of its own — that's search's slot.

### `pi-lean-dimension` (umbrella)
...
No code — just the manifest that bundles portal + search. (The repo root dir is
`pi-lean-portal`; the umbrella package lives at `packages/pi-lean-dimension/` per
...
3. **`/web on/off` semantics widening** — today `BROWSER_TOOL_NAMES` toggles
   ...
   search integration, `/web on` enables **all** web tools including search. This
   ...
   `search` glyph reflects only search tools. So `/web on` with browser tools off
   and search on shows `browser: ● off` / `search: ● on`, not an overloaded
   browser slot. Portal owns both slots (it already knows which tools are
   browser-group vs sibling-group by which set they're in); search needs no
   status logic. The `search` slot is implemented at search integration (step 5),
...
8. **`peerDependencies` for Pi core libs** — every package **that ships code** lists
   ...
   (Codeless packages like `pi-lean-dimension` list no peer deps — they bundle
   transitively.)
...
11. **Status-bar glyph (`ctx.ui.setStatus("browser", …)`)** — **decision: keep
    ...
    added for the search group at search integration (step 5, decision 11)** so the
    browser slot isn't overloaded with search state — `browser` reflects only
    browser tools, `search` reflects only search tools. For v0.1 (portal-only,
    no search), the `browser` slot behaves exactly as today; no churn. (Could
...
12. **Tests** — the current 24 test files / 803 tests **stay almost entirely in
    ...
    + Update `browser-toggle.test.ts` to assert the exact-name-set discovery
      picks up a simulated `web-search` tool registration (search's contracted
      name, decision 10) once added to `SIBLING_TOOL_NAMES`, and to use the new
      `web-toggle-state` key.
    + Add `ship-manifest.test.ts` to portal (and search, and dimension).
...
13. **Existing `pi-browser` private package** — superseded by `pi-lean-portal`.
    ...
    get repointed to `npm:pi-lean-portal` (or `npm:pi-lean-dimension`) on first
    install. Document the switch in the root README.

14. **AGENTS.md / README** — root README covers the suite + install matrix
    (**Mode A** recommended / **Mode B** full-suite / **Mode B-search** search-only
...
15. **Search graceful degradation when unconfigured** — `pi-lean-search` requires
    a running SearXNG instance + a configured API URL (settings key
    `searxng.url` or env var `SEARXNG_URL`). Because dimension bundles search, a user
    who `pi install`s dimension without having SearXNG set up would get a
    non-functional `web-search` tool. **Decision: search registers `web-search`
    ...
    `pi-lean-search` README for self-host vs public-instance options. This keeps
    dimension **safe to install before SearXNG is ready** — the browser works
    immediately, and the search tool self-documents its setup. It also gives a
    portal-only user who later adds search a clear first-use path. Search-side
...
### `pi-lean-dimension/package.json`

```jsonc
{
  "name": "pi-lean-dimension",
  "version": "0.1.0",
  "description": "Pi extension. The full pi web-tools suite — interactive browsing (pi-lean-portal) and SearXNG search (pi-lean-search), unified under /web.",
...
5. **Add `pi-lean-search`** package (your existing searxng code); register its tool
   ...
   reflects only search tools. Decide at implementation time whether the `search`
   slot renders when search isn't installed (likely: only set it if `web-search`
   is present in `pi.getAllTools()`). **Implement graceful degradation**
   (decision 15): search registers `web-search` regardless of config; on first
   call with no `searxng.url` / `SEARXNG_URL`, returns a clear setup message
   pointing at the README — keeps dimension safe to install before SearXNG is ready.
   Add search's `ship-manifest.test.ts`.
6. **Wire `pi-lean-dimension`** umbrella package; verify one
   `pi install npm:pi-lean-dimension` loads both packages with a single `/web`
   (no `/web:1`/`/web:2`).
...
8. **Docs** — root README with install matrix (Mode A recommended /
   Mode B full-suite / Mode B-search search-only / Mode C v0.2), per-package
   ...
   (decision 14); document SearXNG setup requirement for Mode B / B-search.
...
| # | Decision | Resolution |
|---|---|---|
| 1 | npm scope vs. unscoped names | **Unscoped**: `pi-lean-portal`, `pi-lean-search`, `pi-lean-dimension` (unique enough to resist squatting) |
...
| 12 | `/web status` grouping | **v0.1: simple list, no grouping** — `/web status` lists tools by name with on/off state (current behavior, generalized to include search tools). The `packageFromSourceInfo()` helper (no `packageName` field on `sourceInfo`; Mode A vs Mode B path layouts differ) is deferred to v0.2. Downgrades §2.2 to match §4's "optional." Resolves reviewer NOTE-2. |
| 13 | `/web off` re-enable behavior | **Accept for v0.1** — `/web off` reconstructs the active set from the full registry, so it re-enables any search tool the user had manually toggled off (pre-existing browser behavior, widened by the additional sibling name). Revisit at search integration if it matters. Resolves reviewer NOTE-7. |
...
| 14 | Recommended install | **`pi-lean-portal`** (Mode A) — least friction, works fully after `npx playwright install chromium firefox`. `pi-lean-dimension` (Mode B) reframed as the full-suite power-user option — requires a running SearXNG server + URL config, which is setup friction most users won't want up front. `pi-lean-search` (Mode B-search) is search-only with the same SearXNG requirement. The "recommended" label should match the install that works fully out of the box. Architecture unchanged; this is a re-ranking of the install matrix. |
| 15 | Search graceful degradation when unconfigured | **Search registers `web-search` normally regardless of config; on first call with no `searxng.url` / `SEARXNG_URL` set, returns a clear setup message** (not a silent failure or thrown error) pointing the user at settings + the README (self-host vs public SearXNG instance). Keeps `pi-lean-dimension` safe to install before SearXNG is ready — the browser works immediately, and `web-search` self-documents its setup. Also gives portal-only users who later add search a clear first-use path. Search-side design; implemented at step 5. |
...
+ **Recommended install = `pi-lean-portal`** (decision 14) — the install
  that **works fully out of the box** after one Playwright command. Dimension
  (Mode B) is the full-suite power-user option, gated on a running SearXNG
  server + URL config; search degrades gracefully (decision 15) so dimension doesn't
  break when SearXNG isn't set up yet — the browser works immediately and
  `web-search` self-documents its setup on first call.
+ **Umbrella `pi-lean-dimension`** uses Pi's `bundledDependencies` +
  ...
  bundling, and ships no umbrella package. `pi-lean-dimension` is the one genuinely
