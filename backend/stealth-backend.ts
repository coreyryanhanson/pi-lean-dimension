...
class StealthBridge {
		...
		this._initPromise = new Promise<void>((resolve, reject) => {
			...
			this.process.on("exit", (code) => {
				if (code !== 0 && !this.initialized) {
					this._initPromise = null;
					reject(
						new Error(`Bridge process exited with code ${code} during startup`),
					);
				}
			});
		});
	...
	async init(taskId: string): Promise<void> {
		...
		this.taskId = taskId;
		await this.ensureProcess();
		const resp = await this.call("init", {
			seed: generateSeedFromTaskId(taskId),
		});
...
const _elementCaches = new Map<
	string,
	Map<string, { role: string; name: string; level?: number }>
>();
...
function lookupRef(
	taskId: string,
	ref: string,
): { role: string; name: string; level?: number } | null {
...
export async function navigate(
	...
	try {
		...
		const navResult = navResp.result as {
			url: string;
			title: string;
			statusCode: number;
		};
		...
		const { text: snapshotText, count: elementCount } = cacheSnapshot(
			taskId,
			snapRaw,
		);
...
export async function click(
	...
	if (!node) {
		return {
			success: false,
			error: `Element ${ref} not found in cache. Refresh with browser-snapshot first.`,
		};
	}
...
export async function screenshot(
	taskId: string,
): Promise<StealthScreenshotResult> {
	try {
		const bridge = getBridge(taskId);
		...
		return {
			success: true,
			dataUri: (resp.result as { dataUri: string }).dataUri,
		};
	} catch (err: unknown) {
		return {
			success: false,
...
export async function getConsoleMessages(
	...
	try {
		...
		return (
			(resp.result as { messages?: Array<{ type: string; text: string }> })
				.messages ?? []
		);
	} catch {
		return [];
...
export async function evaluate(
	...
	try {
		...
		return {
			success: true,
			result: (resp.result as { result?: unknown }).result,
		};
	} catch (err: unknown) {
		return {
			success: false,
